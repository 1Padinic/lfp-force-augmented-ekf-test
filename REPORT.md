...
*A closing note on how this was built: the simulation and plotting code was written using AI, highly supervised — though the author would rather it hadn't needed to be that way. Yes, lazy. The problem framing, the ground-truth modeling choices, which literature to borrow from, the decision to redo the baseline once the original comparison was called out as unfair, and every interpretation and limitation in this report, are the author's own.*
